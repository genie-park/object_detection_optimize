...
def optimize_model(config_path,
    ...
    if config.model.HasField('ssd'):
        ...
        if override_resizer_shape is not None:
            ...
            config.model.faster_rcnn.image_resizer.fixed_shape_resizer.width = override_resizer_shape[
                1]
            print("***************************8 image_resizer " + str(config.model.faster_rcnn.image_resizer))
    ...
    with tf.Session(config=tf_config):
        with tf.Graph().as_default():
            exporter.export_inference_graph(
                ...
                input_shape=[max_batch_size, override_resizer_shape[0], override_resizer_shape[1], 3])
    ...
    if output_path is not None:
        ...
        with tf.Session(graph=tf.Graph()) as session:
            ...
            tf_num_detections = session.graph.get_tensor_by_name(NUM_DETECTIONS_NAME + ':0')

            tf.saved_model.simple_save(session, export_dir, 
		inputs  = {'inputs': tf_input}, 
                outputs = {BOXES_NAME  : tf_boxes, CLASSES_NAME : tf_classes, 
			   SCORES_NAME : tf_scores, NUM_DETECTIONS_NAME : tf_num_detections})
